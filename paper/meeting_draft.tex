...
texture.  This is an interim report.  Results so far cover the RED-CNN
backbone across three regimes on a subset of the TCIA
LDCT-and-Projection-Data; the remaining architectures and self-supervised
configurations are in progress.  We recently identified and fixed an
...
flow-matching model, trained under five modes---supervised, Noise2Void,
Noise2Sim, ZS-N2N, and Filter2Noise.  The results below report the RED-CNN
backbone; the remaining architectures are queued in the same sweep.
...
\begin{table*}[t]
  \centering
  \caption{\emph{Preliminary} validation results on a TCIA
           LDCT-and-Projection-Data subset (patient-level split, seed 0;
           RED-CNN backbone, 10 training epochs).  Means over the
           validation set on full slices via overlapped tiling.
           $\uparrow$/$\downarrow$: higher/lower is better.  \emph{LDCT
           input} is the noisy scan with no denoising (identity baseline).
           \textbf{Bold}: best denoiser per metric.}
  \label{tab:results}
  ...
  \end{tabular}
  \smallskip\\
  \footnotesize{These numbers predate the overlapped-inference border fix
  (Sec.~3) and are therefore biased pessimistically on PSNR/RMSE; a
  corrected re-run is in progress.  Each row is a single run at seed 0;
  per-patient and multi-seed standard deviations, and the remaining
  architectures (DnCNN, U-Net, CTformer, CFM) and regimes (ZS-N2N,
  Filter2Noise), will be added as runs complete.}
\end{table*}

What we can say already, with the caveat that the absolute PSNR/RMSE values
are biased by the border artefact: (i)~the supervised RED-CNN improves SSIM
over the noisy input ($0.924$ vs.\ $0.893$), so it is genuinely recovering
structure rather than merely smoothing; (ii)~across regimes the ordering is
the expected one---supervised $>$ Noise2Sim $>$ Noise2Void on SSIM, with
the correlated-noise-aware Noise2Sim ahead of the blind-spot Noise2Void;
and (iii)~the low-dose input here is already close to full-dose
(baseline PSNR $31.4$, RMSE $2.7\%$), a high floor that makes pixel-level
gains hard and motivates the perceptual GMSD/NPS metrics.  We will revisit
all absolute claims once the corrected harness has re-run.
