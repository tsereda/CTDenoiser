...
\begin{table*}[t]
  ...
  \caption{\textbf{Flow-vs-regression control} (\texttt{sweep\_flow\_vs\_reg.yml}):
           ...
           that neither estimator invents structure.  $\uparrow$/$\downarrow$:
...
\begin{table*}[t]
  ...
  \caption{\textbf{Main benchmark sweep} (\texttt{sweep.yml}): architecture
           ...
           variant).  The four trained backbones (RED-CNN, U-Net, DnCNN,
           CTformer) are complete over $3$ seeds and reported as
           mean\,$\pm$\,s.d.; CFM is still training and pending (``---'').  The
  ...
  \begin{tabular}{llrccccc}
    ...
    CFM~\cite{lipman2022flow}        & Supervised & $2.11\mathrm{M}$ & \emph{training} & --- & --- & --- & --- \\
    \midrule
...
be attributed to the objective.  Completing CFM across all three seeds in the
main benchmark sweep (Table~\ref{tab:sweep-benchmark}) is the immediate next
step; CTformer is now complete there and, notably, gains only $+1.77$\,dB under
supervision---well short of the CNN backbones---so the transformer does not help
at this budget.
...
completion of the CFM runs across all three seeds of the main benchmark sweep
are in progress.  For the proposed self-supervised
