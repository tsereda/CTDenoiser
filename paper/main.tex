...
Noise2Noise estimator by construction.  All code, model definitions, and
...
\begin{table*}[t]
  ...
  \caption{Validation results on TCIA LDCT (abdomen; patient-level split,
           ...
           targets, so it has no blind-spot or similarity variant.}
  \label{tab:results}
...
\paragraph{Hypothesis and ablation.}
...
$k$-step inference, sensitivity to $r$, and comparison against direct
Noise2Sim and the supervised upper bound under the harness above.  A
positive multi-step effect is the contribution; a null effect narrows the
claim to the correlated-noise-aware pairing.
...
truncated CTformer/CFM runs are in progress.  The proposed self-supervised
flow (Section~\ref{sec:method}) is described but not yet evaluated; its
ablation against Noise2Sim is the primary planned experiment.
...
construction.  All code is released to facilitate reproducibility.
