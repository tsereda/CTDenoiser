...
gap, we prove that the velocity field of an \emph{unconditional} rectified flow,
trained on two noisy observations of the same signal, recovers the Noise2Noise
posterior mean in a \emph{single} step with no clean targets---and that any
multi-step integration provably departs from it.  Both hold empirically: a
matched-pairing experiment shows one-step flow ties direct regression while
multi-step Euler integration monotonically erodes it, so the decorrelated
\emph{pairing}, not the flow machinery, is the useful idea---a one-step regressor
on those pairs is our best label-free result ($+1.99$\,dB) and iteration only
hurts.  Finally, a task-based evaluation with a
...
that multi-step integration monotonically worsens it---both confirmed by a
matched-pairing experiment in which one-step flow ties direct regression while
iteration erodes it, so the decorrelated pairing, not the flow, is what helps
(Section~\ref{sec:method}); and
