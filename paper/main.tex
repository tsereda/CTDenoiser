...
Table~\ref{tab:detect} shows four things.
...
visible in the qualitative comparison (technical appendix).
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.82\textwidth]{steps_departure.pdf}
  \\[4pt]
  \includegraphics[width=0.82\textwidth]{steps_departure_natural.pdf}
...
$3$-seed and patient-level; the supervision-regime ordering, and the
detectability erosion, additionally reproduce over $3$ seeds on an
independent \emph{simulated} low-dose source of the same anatomy
and on a second \emph{anatomy} (chest), so neither is tied to a single
acquisition, noise model, or anatomy.  Broader multi-scanner and multi-anatomy replication is the natural
next step for the CT-specific numbers.  The \emph{theoretical}
...
(technical appendix), so they are not artefacts of CT or of correlated noise.
The per-image methods (ZS-N2N, Filter2Noise) sit essentially at the noisy floor
and are reported single-split; their $3$-seed and detectability numbers are
