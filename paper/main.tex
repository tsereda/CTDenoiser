...
\paragraph{Per-fold stability and convergence.}
The benchmark reports each regime as a $3$-seed mean; to confirm the ordering is
stable \emph{per fold} and established early in training, we repeat the RED-CNN
and DnCNN comparison on the same three folds at a reduced 15-epoch budget
(Table~\ref{tab:cv}), reporting the gain over each fold's \emph{own} input floor
($31.44/30.04/29.27$\,dB) averaged across folds ($\pm$ s.d.).  The ordering is
preserved with a tight per-fold spread: supervised gains $+4.0$ to $+4.2$\,dB
($\pm0.22$); Noise2Sim is again the only label-free method to clear the floor by
a decisive margin ($+1.74\pm0.09$\,dB on RED-CNN); and Noise2Void, Filter2Noise,
and ZS-N2N stay within noise of the input ($|\Delta\text{PSNR}|\le0.32$\,dB).
Absolute PSNR at 15 epochs is already within $\sim$0.1\,dB of the 50-epoch
benchmark (RED-CNN supervised $34.43$ vs.\ $34.50$\,dB), so the regime
ordering---the paper's central claim---is established early and holds on every
fold.
...
a direct Noise2Sim regressor to isolate \emph{why} SSFlow trails Noise2Sim; and
Table~\ref{tab:sweep-periimage} (\texttt{sweep\_periimage.yml}) gives the
per-image grid, cells (``---'') populated as that sweep finishes.
