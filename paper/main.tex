...
\begin{figure*}[t]
  ...
  \\[4pt]
  \IfFileExists{figures/steps_departure_natural.pdf}{%
    \includegraphics[width=\textwidth]{figures/steps_departure_natural.pdf}}{%
    \fbox{\parbox{0.97\textwidth}{\centering\vspace{2.2em}\emph{figures/steps\_departure\_natural.pdf}\\[2pt]
      generate with \texttt{python scripts/figure\_steps.py <natural\_hallucination\_export.csv> {-}{-}regression-ref 4.09}\vspace{2.2em}}}}
...
\begin{table*}[t]
  ...
  \caption{\textbf{Robustness to a second LDCT source} (\texttt{sweep\_sim\_ldct.yml}):
           ...
           best $\Delta$PSNR per supervision block.  $^{\dagger}$CTformer is a
           single-seed provisional entry and CFM is pending; both complete when the
           sweep finishes.  $\uparrow$/$\downarrow$: higher/lower is better.}
  \label{tab:sim-ldct}
  \begin{tabular}{llrccccc}
    \toprule
    \textbf{Model} & \textbf{Supervision} & \textbf{Params} &
    ...
    CTformer$^{\dagger}$~\cite{wang2022ctformer} & Supervised & $0.40\mathrm{M}$ & 30.23 \,($+1.99$) & $0.855$ & $0.0309$ & $0.363$ & $0.0098$ \\
    \midrule
    ...
    CTformer$^{\dagger}$~\cite{wang2022ctformer} & Noise2Sim~\cite{niu2020noise2sim} & $0.40\mathrm{M}$ & 28.92 \,($+0.68$) & $0.806$ & $0.0359$ & $0.352$ & $0.0133$ \\
    \midrule
...
$3$-seed and patient-level but on the abdomen window only; a second anatomy is the
primary remaining check for the CT-specific numbers, though the
supervision-regime ordering already reproduces on an independent, simulated
low-dose source of the same anatomy (Table~\ref{tab:sim-ldct}).  The \emph{theoretical}
