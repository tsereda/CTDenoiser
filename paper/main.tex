...
% copyright slug (that slug belongs only on an accepted camera-ready).  For an
% anonymous conference submission, switch this back to [submission].
...
% Bind the technical appendix into this document (single-file arXiv upload).
% For a page-limited venue where the appendix does not travel with the
% submission PDF, set \includeappendixfalse and build supplementary.tex instead.
...
the opposite for label-free denoising.  An unconditional rectified
...
provably departs from it---exactly so in a tractable Gaussian model---because
...
without ever fabricating structure.  This failure grows
...
What does the work is therefore the decorrelated pairing, not the flow
...
low-dose CT.  Among label-free methods only the correlated-noise-aware Noise2Sim
...
pixel-independence these methods assume.  Under a task-based channelized
Hotelling observer, the $\sim$4\,dB PSNR that supervised denoisers do gain
erodes low-contrast lesion detectability rather than improving it, a shift
PSNR and SSIM miss.  All code, model definitions, and data-loading utilities are
released openly at \url{https://github.com/tsereda/CTDenoiser}.
\end{abstract}
...
regression pass.  We show that for label-free denoising this assumption is
provably counter-productive.  Trained on two independent noisy observations of the
same signal (the Noise2Noise setting), an unconditional rectified flow has
...
time monotonically increases denoising error, because the flow's terminal
...
structure.  We establish this on low-dose computed tomography (CT), where errors
carry clinical weight: CT's spatially correlated reconstruction noise breaks the
pixel-independence assumption behind blind-spot self-supervision, and a task-based
observer shows the fidelity gains denoisers do achieve can erode diagnostic
...
alternative that models the full distribution of plausible clean
images rather than a single point estimate.  The harder regime is
label-free denoising, where paired full-dose targets are
...
The benchmark serves the theoretical result rather than standing as an end in
itself: its matched-data, matched-backbone control is what lets the
label-free flow-versus-regression comparison below isolate the estimator
from the pairing, and turn the one-step-optimality claim into a controlled
test rather than an isolated observation.
...
counterintuitive corollary that the degradation grows as the velocity is
...
benchmark's PSNR/SSIM gains erode low-contrast lesion detectability without
...
RED-CNN~\cite{chen2017lowdose}, a residual encoder-decoder with multi-level skip
connections, remains a standard baseline, and DnCNN~\cite{zhang2017beyond} showed
that a flat Conv-BN-ReLU stack with a single residual (noise-subtraction)
prediction denoises well and transfers to CT.  CTformer~\cite{wang2022ctformer}
adds a U-shaped transformer whose Token2Token Dilation blocks give long-range
self-attention without the quadratic cost of global attention.

\paragraph{Generative approaches.}
Score-based diffusion~\cite{song2021scorebased} has been applied to CT
denoising~\cite{gao2023corediff} with strong perceptual quality but hundreds to
thousands of neural function evaluations (NFEs); conditional flow
matching~\cite{lipman2022flow,albergo2022building} cuts this to 10--50 NFEs via
straight-line probability paths.  Poisson Flow Consistency Models
(PFCM)~\cite{hein2025pfcm} distil this to a single step but report large
image-to-image variability from the unconstrained generative process, motivating
fidelity-preserving sampling---which our self-supervised flow enforces by
construction (Section~\ref{sec:method}).  The perception--distortion
trade-off~\cite{blau2018perception} is the formal reason a generative denoiser can
look realistic while being unfaithful.  Closest to us, an optimally trained
...
posterior-mean-then-transport estimators (PMRF~\cite{ohayon2025pmrf},
InDI~\cite{delbracio2023indi}) separate MMSE regression from generative transport;
Section~\ref{sec:method} pushes this to the label-free regime and finds the
transport not merely optional but harmful to the denoising objective.

\paragraph{Self-supervised and zero-shot denoisers.}
When paired targets are unavailable, Noise2Noise~\cite{lehtinen2018noise2noise}
recovers the clean signal in expectation by mapping between two independent noisy
observations.  Noise2Void~\cite{krull2019noise2void} drops the second observation
via a blind-spot scheme but assumes pixel-independent noise---violated by
CT's spatially correlated reconstruction noise---as does the single-image
Neighbor2Neighbor~\cite{huang2021neighbor2neighbor}.  Two lines target correlated
noise: Noise2Sim~\cite{niu2020noise2sim} maps between centre pixels of
self-similar non-local patches, staying asymptotically equivalent to supervised
learning, and Zero-Shot Noise2Noise (ZS-N2N)~\cite{mansour2023zsn2n} trains a tiny
per-image network from a downsampled pair.  AP-BSN~\cite{lee2022apbsn} breaks
correlation with an asymmetric train/inference stride, while
Filter2Noise~\cite{sun2025filter2noise} and WIA-LD2ND~\cite{zhao2024wia} give
interpretable single-image and wavelet-domain LDCT variants.
Noise2Score~\cite{kim2021noise2score} unifies these as score estimation whose
Tweedie posterior mean is the same MMSE target our flow reaches in one step
(Section~\ref{sec:method}).  Our benchmark includes Noise2Void, ZS-N2N, and
Filter2Noise, and adds the correlated-noise-aware Noise2Sim.

\paragraph{Task-based evaluation and metrics.}
\label{sec:rw-taskbased}
PSNR and SSIM are poor proxies for diagnostic quality: they misrank restoration
algorithms~\cite{breger2025reassess}, do not track radiologist perception of
denoised CT~\cite{lee2025ldctiqa}, and penalise generative models that
hallucinate plausible high-frequency detail---motivating perceptual and texture
measures such as LPIPS~\cite{zhang2018unreasonable} and the noise power spectrum
(NPS)~\cite{solomon2012characterization}.  The stronger standard is
task-based: channelized Hotelling observers (CHO) estimate detectability
($d'$, AUC) on signal-known-exactly / background-known-statistically tasks.
Bhadra et al.~\cite{bhadra2021halluc} formalised hallucination via the
measurement/null-space decomposition and Tivnan et al.~\cite{tivnan2024halluc} a
Hallucination Index; applied to denoisers, Li et al.~\cite{li2021detection} and
Kc \& Zeng~\cite{kc2024lgcho} showed deep denoising can destroy
task-relevant information---even beating full-dose on PSNR/SSIM while losing
low-contrast detectability---motivating the detectability evaluation of
...
\paragraph{RED-CNN, DnCNN, CTformer, U-Net.}
RED-CNN~\cite{chen2017lowdose} is a five-layer residual encoder-decoder
($5\times5$ convolutions, $96$ filters, skip connections at three depths, ReLU).
DnCNN~\cite{zhang2017beyond} is $17$ layers of $3\times3$ Conv-BN-ReLU that
predict the noise $n$ and return $\hat{x} = x - n$.
CTformer~\cite{wang2022ctformer} tokenises with a strided convolution and applies
a symmetric encoder-bottleneck-decoder of Transformer blocks interleaved with
Token2Token Dilation (cyclic spatial shift plus dilated re-tokenisation before
self-attention).  Our U-Net baseline (three encoder stages, a two-layer
bottleneck, and a bilinear-upsampling decoder with concatenated skip connections)
shares the body of our CFM velocity network, so U-Net (MSE) vs.\ CFM
(flow-matching) isolates the training objective at matched capacity.
...
edge.  An earlier version that discarded the margin on all sides
...
SSIM may miss.  RSR is a frequency-domain error ratio against the
...
($\Delta$) of each metric relative to the unprocessed LDCT input,
...
\begin{table*}[t]
  ...
  \caption{\textbf{Main benchmark} (\texttt{sweep.yml}): validation results on
           ...
           LDCT input floor in parentheses (mean floor $30.25$\,dB).  All
           five architectures are complete over $3$ seeds.  CFM requires paired
           targets, so it has no blind-spot or similarity variant.  The
           label-free SSFlow and its one- vs.\ multi-step ablation are in
...
reduced 15-epoch replication on the same three folds reproduces it---supervised
...
signal-present low-dose image is literally $x_\text{low} + s$ for a known
...
slices per run), extract present/absent ROIs at the input, denoised
and clean stages, channelise with ten Laguerre-Gauss channels, and compute
...
SSFlow/CFM detectability, which can fabricate as well as erode, is reported
separately.
...
\begin{table*}[t]
  ...
  \caption{Task-based detectability on the completed benchmark models
           ...
           preserved, $d'_\text{denoised}/d'_\text{input}$
           (mean\,$\pm$\,s.d.; $1.0=$ unchanged, $<1=$ eroded); the denoised
           noise-power-spectrum radial centroid (input $0.112$; lower $=$ more
           low-frequency ``waxy'' texture); and the fabrication $d'$ from
           a CHO run on signal-absent ROIs (input floor $0.028$; $>$floor
...
headroom is small---the clean reference exceeds the noisy input by only
$\sim$2\% ($d'=6.18$ vs.\ $6.06$ at $160$\,HU)---which is the clinically relevant
regime: at a realistic LDCT dose supra-threshold lesions are already detectable,
so the question is whether denoising preserves detectability, not whether
it recovers it.  \textbf{Second}, though these models gain $4.0$--$4.3$\,dB PSNR
(supervised) and $1.7$--$1.8$\,dB (Noise2Sim) (Table~\ref{tab:results}), none
improves detectability at the supra-threshold point:
$d'_\text{denoised}<d'_\text{input}$ in all $27$ runs (a $7$--$8\%$ loss at
$160$\,HU), none exceeds the clean ceiling, and the fabrication $d'$ stays at its
floor ($0.022$--$0.033$ vs.\ $0.028$)---the denoisers erode detectability
without fabricating it.  The loss concentrates at high contrast (within
$1$--$2\%$ near the $40$\,HU threshold), precisely where PSNR/SSIM report their
largest gains.  \textbf{Third}, the models that win most on PSNR shift the NPS
centroid furthest toward low frequency ($0.112\!\to\!0.087$, $22\%$, vs.\
$0.112\!\to\!0.109$ for Noise2Void)---the blotchy ``waxy'' texture radiologists
distrust (qualitative comparison, Appendix~\ref{app:qual}).  \textbf{Fourth},
differences between regimes are not resolved at this sample size (preserved
ratios at $160$\,HU $0.934{\pm}0.033$ supervised, $0.919{\pm}0.030$
Noise2Sim/Noise2Void overlap within one s.d.), so the data support the
erosion claim---every regime sits below $1.0$---but not a regime-vs-regime
ranking; only the NPS-centroid shift separates supervised from blind-spot.  The
preserved ratio cancels the large between-split variation in absolute $d'$
($\pm1.31$ at $160$\,HU) and is stable across seeds.  None of this appears in PSNR
or SSIM, which rise monotonically; task-based $d'$ is a necessary complement, not
an optional one.
...
results: the velocity field of an unconditional rectified flow, trained on
two noisy observations of the same signal, has a minimiser that recovers the
Noise2Noise posterior mean in a single step (Section~\ref{sec:rf-n2n})---no
clean targets, no ODE integration---and any multi-step integration provably
...
similarity-based construction of the noisy pairs that keeps the paired
...
unconditional velocity network $v_\theta(z,t)$ (inputs: the
...
Our result connects two identities not previously brought together in the
label-free setting: the flow-matching denoising identity---an optimally trained velocity
...
the former assume a clean target $x_1$; the label-free, symmetric
two-noisy-realisation case ($\mathbb{E}[x_1\mid s]=\mathbb{E}[x_0\mid s]=s$) is
what is new here, and it is what makes the one-step map a self-supervised MMSE
...
restoration objective---the opposite of the Ambient Diffusion
regime~\cite{daras2023ambient}, where noisy-only training deliberately
samples the clean distribution rather than collapsing to its mean.  It is
a population-level identity at the optimum; how far a finite-capacity,
finite-step flow departs from it is the empirical question
...
network reproduces the noisy $x_1$ rather than averaging over it,
...
retrieve its $K$ most similar patches elsewhere in the slice, excluding
...
input; we do not integrate to the noisy endpoint distribution.

\paragraph{Multi-step refinement provably departs from the optimum.}
The one-step readout is exact only at $t{=}0$; integrating further
provably increases denoising error, because the flow's terminal marginal is the
noisy law $p_1$, not the clean signal.  We make this precise in the
...
$t$.  The one-step estimate ($t{=}0$) is therefore exactly optimal, and each
...
exact continuous flow under-shrinks for every $t>0$, and Euler
...
better-trained velocity integrates more faithfully toward the noisy
endpoint, so multi-step degrades as training proceeds---exactly the
counter-intuitive trend in Table~\ref{tab:ssflow-steps}.  The one-step posterior
mean is thus the provably optimal readout on N2N pairs, and the cost of iteration
is now made precise rather than merely observed.
...
multi-step refinement improves the structured residual that one-step
...
\emph{(i) The decorrelated pairing is what matters.}  PSNR follows a
...
just beyond the FBP correlation length is what helps.
...
PSNR falls monotonically with the step count, from $+1.88$ (one-step) to
...
Swapping the estimator on the same pairs (Table~\ref{tab:flow-vs-reg})
...
caveat of Section~\ref{sec:rf-n2n}), so the decorrelated-pairing one-step
...
distortion / task criterion: consistent with matched-data CT
...
integration erodes PSNR monotonically, $+3.9\!\to\!+1.3$\,dB over
$1/4/8/20$ steps---a steeper fall than on CT, as expected when the
one-step gain is larger.  \emph{(iii)} Consistent with $r$ being the
correlated-noise knob, the exclusion radius barely matters here
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.74\textwidth]{steps_departure.pdf}
  \\[4pt]
  \includegraphics[width=0.74\textwidth]{steps_departure_natural.pdf}
  \caption{\textbf{The finite-step departure, predicted and confirmed.}
           ...
           provably increases error (Eq.~\eqref{eq:kappa}).  \emph{(b)}
...
(\texttt{sweep\_hallucination.yml}); its regression control
(\texttt{sweep\_flow\_vs\_reg.yml})---holding the similarity pairing fixed and
swapping the flow estimator for a direct Noise2Sim regressor to isolate whether
the flow or the pairing is what matters---is in Appendix~\ref{app:method-tables}.
...
\begin{table*}[t]
  ...
  \caption{\textbf{SSFlow inference-steps ablation} (\texttt{sweep\_hallucination.yml}):
           ...
           $30.25$\,dB input floor.  PSNR degrades monotonically with the
           ...
           input floor ($\approx$$0.028$) at every step count: iteration
...
(Appendix~\ref{app:method-tables}).

\paragraph{Metric selection.}
PSNR and SSIM penalise any pixel-level deviation, disadvantaging generative
models; recent reassessments show both misrank restoration algorithms on medical
...
judgements~\cite{xue2014gradient}, and RSR is interpretable as a frequency-domain
characterisation of residual noise.  The stronger standard---task-based
detectability via a model observer~\cite{li2021detection,kc2024lgcho} with
explicit hallucination quantification~\cite{bhadra2021halluc,tivnan2024halluc}---is
reported in Section~\ref{sec:detect}: the CHO confirms the PSNR/SSIM gains
overstate task performance, with low-contrast detectability eroded rather than
improved.  We recommend future CT-denoising work report task-based $d'$ alongside
these metrics.

\paragraph{Limitations.}
The clinical evaluation is $3$-seed and patient-level; the
supervision-regime ordering and the detectability erosion reproduce over $3$
seeds on a simulated low-dose source (same anatomy) and on a second
anatomy (chest), so neither is tied to one acquisition, noise model, or
anatomy---multi-scanner replication is the natural next step.  The
theoretical claims are less domain-bound, reproducing on natural images
under i.i.d.\ Gaussian noise.  The per-image methods (ZS-N2N, Filter2Noise) are
reported single-split, as each CHO realisation would retrain their per-image
network thousands of times.  The finite-step departure is proved exactly only in
the jointly-Gaussian model---a general finite-capacity bound is open, though the
CT and natural-image experiments both show the predicted monotone erosion.
Detectability is reported for the deterministic denoisers and, via the
fabrication-vs-steps pass, the generative SSFlow; a broader hallucination analysis
...
methodological contribution is a proof that an unconditional rectified
...
fidelity, so the decorrelated pairing, not the flow machinery, is what
carries the method, and a one-step regressor on those pairs is our best label-free
result ($+1.99$\,dB).  The practical implication is direct: because the multi-step
penalty is provable rather than incidental, a label-free denoiser built for
fidelity should read out the one-step posterior mean rather than integrate a
generative trajectory---iteration buys texture, not accuracy.  All code is released
at \url{https://github.com/tsereda/CTDenoiser} to facilitate reproducibility.

% ===== References (extra pages permitted by AAAI) =====

% NOTE (camera-ready): AAAI requires natbib + aaai2026.bst with a .bib file;
% the inline thebibliography below is acceptable for review but should be
% converted (\bibliographystyle{aaai2026}, \bibliography{refs}) on acceptance.
...
% ===== Technical appendix (bound in for the preprint; see the toggle above) =====
\ifincludeappendix
