...
recover $\sim$4\,dB PSNR over the noisy input and finish within $0.3$\,dB of one
...
optimum without clean targets, \emph{and} a finite-step departure result proving
that multi-step integration monotonically worsens it---so on noisy pairs the
flow is dominated by direct regression, which our matched-pairing experiment
confirms (Section~\ref{sec:method}); and
...
these methods rely on.  \textbf{Third}, the $3$--$4$\,dB gap between the best
self-supervised method and the supervised upper bound quantifies the headroom
...
help; it hurts}---exactly as the finite-step departure of
Eq.~\eqref{eq:kappa} predicts.  Four-step Euler inference is uniformly below
one-step at matched training and \emph{degrades} as training proceeds (the
training paradox of the Proposition), falling to $+0.52$,
...
\emph{(iii) The pairing is the contribution; the flow is not.}  Holding the
pairing fixed and swapping the estimator (Table~\ref{tab:flow-vs-reg}) isolates
the objective: on the \emph{same} decorrelated pairs a direct regressor rises to
$+2.00$\,dB at $r{=}2$---above standard Noise2Sim ($r{=}1$) and the best
label-free result in our benchmark---while the flow stays at $+0.9$--$1.4$\,dB
(early returns, $2$ of $3$ seeds).  So the decorrelated non-local pairing is the
transferable idea, and the rectified-flow machinery is a liability, not an asset:
one-step regression on these pairs is the estimator of choice.  This is the
label-free echo of the supervised U-Net-vs-CFM gap of
Section~\ref{sec:results}---flow trailing regression in both regimes.  The scope
of this claim is the \emph{distortion / task} criterion: consistent with matched-data
CT benchmarks~\cite{eulig2024benchmark}, regression wins on PSNR/SSIM and---by
...
(\texttt{sweep\_hallucination.yml}), filled from the current export with an
explicit \emph{epoch} column because the four-step arm is still training;
Table~\ref{tab:flow-vs-reg} (\texttt{sweep\_flow\_vs\_reg.yml}) is its regression
control, holding the similarity pairing fixed and swapping the flow estimator for
a direct Noise2Sim regressor to isolate \emph{why} SSFlow trails Noise2Sim.  The
...
\begin{table*}[t]
  ...
  \caption{\textbf{Flow-vs-regression control} (\texttt{sweep\_flow\_vs\_reg.yml}):
           ...
           cell-for-cell; the PSNR gain is over each split's own input floor.  The
           decisive read is regression$(r)$ vs.\ flow$(r)$ at matched $r$.
           \emph{Early returns} ($\ddagger$: regression $2$ of $3$ seeds finished)
           give the \emph{flow-is-the-cost} verdict: the regressor \emph{rises} on
           the decorrelated pairs ($+1.75\!\to\!+2.00$\,dB from $r{=}1$ to $r{=}2$,
           lifting standard Noise2Sim), while the flow sits at $+0.9$--$1.4$\,dB on
           the \emph{same} pairs---so the rectified-flow objective, not the pairing,
           is what makes SSFlow trail.  The decorrelated-pairing regressor at
           $r{=}2$ ($+2.00$\,dB) is the best label-free result in the benchmark.
           The fabrication $d'$ (from \texttt{--eval-detectability}) confirms
           neither estimator invents structure ($\approx$ input floor).
           $\dagger$: provisional
  ...
  \begin{tabular}{lcrccccc}
    ...
    Regression & $1^{\ddagger}$ & 32.10 \,($+1.75{\pm}0.20$) & $0.900{\pm}0.021$ & $0.0258$ & $0.164$ & $0.0076$ & $0.031$ \\
    Regression & $2^{\ddagger}$ & \textbf{32.35 \,($+2.00{\pm}0.13$)} & $0.904{\pm}0.021$ & $0.0250$ & $0.160$ & $0.0071$ & $0.033$ \\
    Regression & $3$ & --- & --- & --- & --- & --- & --- \\
    \midrule
...
The benchmark (Table~\ref{tab:results}) is a $3$-seed, patient-level evaluation
on the abdomen window at 50 epochs; multi-anatomy runs and completion of the
per-image (Table~\ref{tab:sweep-periimage}) and flow-vs-regression
(Table~\ref{tab:flow-vs-reg}) sweeps remain in progress.  For the proposed
self-supervised flow (Section~\ref{sec:method}) the four-step inference arm is
still completing (ep.\,38--46), though its one- vs.\ multi-step trend is already
unambiguous; the dedicated multi-seed one-step SSFlow head-to-head at matched
pairing remains to be finalised.  The detectability evaluation
(Section~\ref{sec:detect}) covers the deterministic denoisers on the $3$-seed
benchmark; the generative SSFlow and CFM detectability---where fabrication, not
just erosion, can arise---is in progress.
