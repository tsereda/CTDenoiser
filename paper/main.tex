...
without ever fabricating structure.  This failure \emph{grows
...
What does the work is therefore the decorrelated \emph{pairing}, not the flow
...
via a blind-spot scheme but assumes pixel-\emph{independent} noise---violated by
...
\emph{task-based}: channelized Hotelling observers (CHO) estimate detectability
...
Kc \& Zeng~\cite{kc2024lgcho} showed deep denoising can \emph{destroy}
...
so the question is whether denoising \emph{preserves} detectability, not whether
...
floor ($0.022$--$0.033$ vs.\ $0.028$)---the denoisers \emph{erode} detectability
without \emph{fabricating} it.  The loss concentrates at high contrast (within
...
differences \emph{between} regimes are not resolved at this sample size (preserved
...
\emph{erosion} claim---every regime sits below $1.0$---but not a regime-vs-regime
ranking; only the NPS-centroid shift separates supervised from blind-spot.  The
\emph{preserved} ratio cancels the large between-split variation in absolute $d'$
...
what is new here, and it is what makes the one-step map a \emph{self-supervised} MMSE
...
Swapping the estimator on the \emph{same} pairs (technical appendix)
...
The \emph{clinical} evaluation is $3$-seed and patient-level; the
supervision-regime ordering and the detectability erosion reproduce over $3$
seeds on a \emph{simulated} low-dose source (same anatomy) and on a second
\emph{anatomy} (chest), so neither is tied to one acquisition, noise model, or
anatomy---multi-scanner replication is the natural next step.  The
\emph{theoretical} claims are less domain-bound, reproducing on natural images
...
fidelity, so the decorrelated \emph{pairing}, not the flow machinery, is what
...
% ===== Reproducibility checklist: AAAI places this in the main PDF after the
% references; it does NOT count toward the 7-page limit. =====
...
table captions; results are mean\,$\pm$\,s.d.\ over $3$ seeds, including the
chest and simulated-source robustness checks.
