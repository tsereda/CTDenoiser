...
the objective: on the \emph{same} decorrelated pairs a direct regressor
($3$ seeds) rises to $+1.99$\,dB at $r{=}2$---above standard Noise2Sim ($r{=}1$,
$+1.77$) and the best label-free result in our benchmark---while the flow stays
at $+0.9$--$1.4$\,dB.  So the decorrelated non-local pairing is the
...
\begin{table*}[t]
  ...
  \caption{\textbf{Flow-vs-regression control} (\texttt{sweep\_flow\_vs\_reg.yml}):
           ...
           decisive read is regression$(r)$ vs.\ flow$(r)$ at matched $r$, and the
           regression arm ($3$ seeds, complete) gives the \emph{flow-is-the-cost}
           verdict: the regressor follows an inverted-U in $r$, \emph{rising} on the
           decorrelated pairs to $+1.99$\,dB at $r{=}2$
           ($+1.77\!\to\!+1.99\!\to\!+1.87$ for $r{=}1/2/3$, lifting standard
           Noise2Sim), while the flow sits at $+0.9$--$1.4$\,dB on the \emph{same}
           pairs---so the rectified-flow objective, not the pairing, is what makes
           SSFlow trail.  The decorrelated-pairing regressor at $r{=}2$
           ($+1.99$\,dB) is the best label-free result in the benchmark.
